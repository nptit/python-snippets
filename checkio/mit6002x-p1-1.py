...
class Robot(object):
    ...
    def __str__(self):
        return "Robot: position {} speed {}".format(self.position, self.speed)
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        new_position = current_position.getNewPosition(direction, speed)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    coverage = lambda room: 1.0 * room.getNumCleanedTiles() / room.getNumTiles()

    time, trial_times = 0, []
    for i in range(num_trials):
        robots = [robot_type(room, speed) for _ in range(num_robots)]
        while coverage(room) < min_coverage:
            for r in robots:
                r.updatePositionAndClean()
            time += 1
        trial_times.append(time)

    return 1.0 * sum(trial_times) / num_trials
...
if __name__ == '__main__':
    ...
    speed = 1
    robots = [StandardRobot(room, speed) for _ in range(3)]
    #print robots[0], robots[1], robots[2]

    print runSimulation(3, 1.0, 20, 20, 1.0, 10, StandardRobot)
    print runSimulation(3, 1.0, 20, 20, 1.0, 10, RandomWalkRobot)
