...
def getWordScore(word, n):
    ...
    score = sum(SCRABBLE_LETTER_VALUES[c] for c in word) * len(word)
    if len(word) == n:
        score += 50
    return score
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList and all(c in hand for c in word):
        ...
        if all(freq_word[c] <= hand[c] for c in word):
            return True
    return False
...
if __name__ == '__main__':
    wordList = loadWords()
    #displayHand({'a': 1, 'c': 1, 'r': 2, 't': 1, 'o': 1})
    # playHand({'q': 1, 'i': 1, 'o': 1}, wordList, 3)
    # playHand({'a': 1, 'c': 1, 'r': 2, 't': 1, 'o': 1}, wordList, 6)
