...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        ret = self.points[0].distance(other.points[0])
        for p in range(len(self.points)):
            for q in range(len(other.points)):
                distance = self.points[p].distance(other.points[q])
    ...
    def averageLinkageDist(self, other):
        ...
        ret = 0.0
        divisor = len(self.points) * len(other.points)
        for p in range(len(self.points)):
            for q in range(len(other.points)):
                distance = self.points[p].distance(other.points[q])
                ret += distance
        return ret / float(divisor)
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        newPoints = list(set(c1.points) | set(c2.points))
        self.members.remove(c1)
        self.members.remove(c2)
        self.add(Cluster(newPoints, City))
    def findClosest(self, linkage):
        """ Returns a tuple containing the two most similar
        clusters in self
        Closest defined using the metric linkage """
        import itertools
        combos = list(itertools.combinations(self.members, 2))
        ans = linkage(combos[0][0], combos[0][1])
        c1 = combos[0][0]
        c2 = combos[0][1]
        for i in combos:
            result = linkage(i[0], i[1])
            if result < ans:
                c1 = i[0]
                c2 = i[1]
                ans = result
    ...
    def mergeOne(self, linkage):
        ...
        result = self.findClosest(linkage)
        self.mergeClusters(result[0], result[1])
