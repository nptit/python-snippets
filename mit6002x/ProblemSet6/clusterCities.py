...
class Cluster(object):
    ...
    def maxLinkageDist(self, other):
        ...
        self and the other point is from other. Uses the
        Euclidean dist between 2 points, defined in Point."""
        return max(p1.distance(p2) for p1 in self.points for p2 in other.points)
    ...
    def averageLinkageDist(self, other):
        ...
        distances = [p1.distance(p2) for p1 in self.points for p2 in other.points]
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        nc = Cluster(c1.points + c2.points, c1.pointType)
        self.add(nc)
    ...
    def findClosest(self, linkage):
        ...
        nm = len(self.members)
        res = [(self.members[i], self.members[j], linkage(self.members[i], self.members[j])) for i in range(nm) for j in range(i + 1, nm)]
        sort_res = sorted(res, key = lambda x: x[-1])
        return tuple(sort_res[0][0:2])
...
def test():
    points = buildCityPoints('test2.data', False)
